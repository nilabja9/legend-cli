...
- **Enhanced Model Generation**: AI-powered detection of enumerations, with proper `EnumerationMapping` blocks in mappings
- **DuckDB Support**: Connect Legend to local DuckDB databases via PostgreSQL wire protocol using Buena Vista proxy
- **AI-Powered Documentation**: Automatically generate `doc.doc` descriptions for classes and attributes (enabled by default in enhanced mode)
...
Enhanced mode uses AI to detect enumerations and automatically generates documentation. This is enabled with the `--enhanced` flag.

#### Enumeration Detection

The CLI analyzes your database schema using AI to identify columns that should be modeled as Pure enumerations:

```bash
# Generate enhanced model with enum detection
legend-cli model from-duckdb ./trading.duckdb --enhanced

# Or for Snowflake
legend-cli model from-snowflake MY_DB --schema TRADING --enhanced
```

**What gets detected:**
- Columns with low cardinality (< 20 distinct values)
- Columns with naming patterns like `*_TYPE`, `*_STATUS`, `*_CODE`
...
**EnumerationMapping in Mapping:**

The mapping file includes proper `EnumerationMapping` blocks that map database values to enum values:

```pure
Mapping model::mapping::TradingMapping
(
  model::domain::OrderType: EnumerationMapping
  {
    MARKET: ['MARKET'],
    LIMIT: ['LIMIT'],
    STOP: ['STOP'],
  ...
  model::domain::Order: Relational
  {
    ~mainTable [model::store::Trading]CORE.ORDER
    orderId: [model::store::Trading]CORE.ORDER.ORDER_ID,
    orderType: EnumerationMapping model_domain_OrderType: [model::store::Trading]CORE.ORDER.ORDER_TYPE,
    orderStatus: EnumerationMapping model_domain_OrderStatus: [model::store::Trading]CORE.ORDER.ORDER_STATUS
  }
)
```
...
|--------|---------|-------------|
| `--enhanced` | false | Enable enhanced AI analysis |
| `--hierarchies/--no-hierarchies` | false | Detect class inheritance hierarchies |
| `--enums/--no-enums` | true | Detect enumeration candidates |
| `--constraints/--no-constraints` | false | Generate data constraints |
| `--derived/--no-derived` | false | Detect derived properties |
| `--confidence` | 0.7 | Minimum confidence threshold (0.0-1.0) |

#### Example: Full Enhanced Generation

```bash
legend-cli model from-duckdb ./trading_core.duckdb \
  --enhanced \
  --project-name "Trading-Model" \
  --output ./generated
```

This generates:
- **enumerations.pure**: All detected enumerations with values from the database
- **classes.pure**: Classes with `doc.doc` annotations and enum-typed properties
- **mapping.pure**: Includes `EnumerationMapping` blocks
- **store.pure**, **connection.pure**, **runtime.pure**, **associations.pure**
...
**In enhanced mode, documentation is generated by default** - no need for `--auto-docs`.
...
2. **Create config file** at `~/.legend-cli/.env`:
   ```bash
   mkdir -p ~/.legend-cli
   cat > ~/.legend-cli/.env << 'EOF'
   ANTHROPIC_API_KEY=sk-ant-your-key-here
   LEGEND_SDLC_URL=http://localhost:6900/sdlc/api
   EOF
   ```

   > **Note**: The MCP server searches for `.env` in: current directory, `~/.legend-cli/.env`, and the package directory.
